...
plugins=(git autojump copypath direnv zsh-autosuggestions)
...
pull-all() {
    ...
    do
        cd $dir
        echo "${Blue}Entered $(pwd)${NC}"
        git pull
        echo "${Green}Finished pull for repo in $dir${NC}"
        cd -
    done
}
...
showme () {
    ...
    tmp="/tmp/workspaces/$(xxd -l3 -ps /dev/urandom)"
    mkdir -p "$tmp"
    7z x "$1" -o"$tmp"
    cd "$tmp" || return 1
